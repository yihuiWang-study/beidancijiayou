...
function pickWord() {
  ...
  const remaining = pool.filter((word) => !doneMap[keyFor(word)] || state.mode === "mistakes" || state.speakingFilter === "practice");
...
function render() {
  ...
  const pool = activePool();
  const doneMap = state.mode === "speaking" ? saved.speakingDone || {} : saved.done || {};
  const doneInMode = pool.filter((word) => doneMap[keyFor(word)]).length;
...
function nextWord() {
  ...
  const doneMap = state.mode === "speaking" ? saved.speakingDone || {} : saved.done || {};
  state.current = ordered.find((word) => state.mode === "mistakes" || state.speakingFilter === "practice" || !doneMap[keyFor(word)]) || ordered[0] || null;
  state.answered = false;
  render();
}
...
$("resetProgress").addEventListener("click", () => {
  const saved = readStore();
  if (state.mode === "speaking") {
    activePool().forEach((word) => {
      delete saved.speakingDone[keyFor(word)];
      delete saved.speakingPractice[keyFor(word)];
    });
  } else {
